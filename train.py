...
class VideoDescriptionTrain(object):
    ...
    def __init__(self, config):
        self.train_path = config.train_path_msvd
        self.test_path = config.test_path_msvd
        ...
        self.save_model_path = config.save_model_path_msvd
    ...
    def preprocessing(self):
        ...
        for y in y_data:
            for caption in y['caption']:
                ...
                if len(caption.split()) > 20 or len(caption.split()) < 6:
                    continue
                else:
    ...
    def train_model(self):
        ...
        self.encoder_model.compile(optimizer='adam', loss='binary_crossentropy')
        self.decoder_model.compile(optimizer='adam', loss='binary_crossentropy')

        # saving the models
        self.encoder_model.save(os.path.join(self.save_model_path, 'encoder_model.keras'))
        self.decoder_model.save(os.path.join(self.save_model_path, 'decoder_model_weights.keras'))
        with open(os.path.join(self.save_model_path, 'tokenizer' + str(self.num_decoder_tokens)) + '.joblib', 'wb') as file:
